...
const vehicleZod = z.object({
    ...
    lastValet: z.preprocess(emptyToUndef, z.coerce.date())
    .refine(d => d instanceof Date && Number.isFinite(d.getTime()), "Invalid date")
    .refine(d => d <= new Date(), "Last valet must be in the past or today")
    .optional(),
    mileage: z.coerce.number().min(0).transform(v => Math.max(0, Math.round(v))),
    fuel: z.coerce.number().min(0).max(100).transform(v => Math.round(Math.min(100, Math.max(0, v)))),
    powertrain: z.string().transform(s => s.trim().toLowerCase())
               .pipe(z.enum(['electric','hybrid','petrol','diesel','unknown'])).default('unknown'),
    gearbox: z.string().transform(s => {
    const t = s.trim().toLowerCase();
    return t === 'auto' ? 'automatic' : t;
    }).pipe(z.enum(['automatic','manual','unknown'])).default('unknown')
}).strict()
